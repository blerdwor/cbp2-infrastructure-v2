...
public:
    ...
    my_predictor() : 
		...
		numBimodalEntries = (1 << BIMODAL_PRED_SIZE);
		...
		// Initialize the array that stores TAGE table indices
		tageIndex = new uint32_t[NUM_TAGE_COMPONENTS];    
    	for(uint32_t i = 0; i < NUM_TAGE_COMPONENTS; i++)   
    		tageIndex[i] = 0;
     
	 	// Initialize the array that stores TAGE tags for each TAGE component
    	tageTag = new uint32_t[NUM_TAGE_COMPONENTS]; 
    	for(uint32_t i = 0; i < NUM_TAGE_COMPONENTS; i++)
        	tageTag[i] = 0;
	}
    ...
    branch_update *predict(branch_info &b) {
		if (b.br_flags & BR_CONDITIONAL) {
			...
			uint32_t bimodalIndex = (b.address) % (numBimodalEntries); 
